...
filePath.addEventListener('change', async (e) => {
  ...
  try{
    ...
    const serverAddress =  `http://127.0.0.1:${response.port}`
    serverInfoDiv.innerHTML = `
        <h3>Szerver elérése:</h3>
        <p>http://127.0.0.1:${response.port}</p> 
        <h3>Elérhető végpontok:</h3>
        <ul class="list-group">   
            ${response.endpoints.map(e => `<li class="list-group-item list-group-item-action list-group-item-primary">${e.method} <a>${serverAddress}${e.path.replaceAll("<","&lt;").replaceAll(">", "&gt;")}</a></li>`).join('')}
        </ul>
    `;
